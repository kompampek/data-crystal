...
CCCCCC        RATIO=E22(k)                 !!YV!!!!!!!
         RATIO=FN1(k)                 !!YV
CCCCCCCC       write(*,*)'EEEEratio==',E1(k),E22(k),RATIO   !!YV
        IF (RATIO.lt.200) then     !!YV
CCCC      FN1(k)=FO1(k)              !!YV write FO
       go to 31                   !!YV     
       else                       !!YV
CC      WRITE(*,*)'ratio,FC2(k1),FN1(k)', ratio,FC2(k1),FN1(k)   !!YV
CC       FN1(k)=FC2(k1)             !!YV  write FH_c
CC        EE=E2(k1)                !!YV write E_heavy atom
         FN=FC2(k)                 !!YV  write FH_c
         EE=E22(k)                  !!YV write E_heavy atom
         PN=PN2(k)                  !!YV
         PH_ERR=PH_ERR+ABS(PN1(k)-PN) !YV
         INUM=INUM+1                      !YV 
CCCCCCCCCCCCCC       write(*,*)'EE===',EE,E2(k1)        
       iflag=1                    !!YV   
       end if                     !!YV
...
CCCCCCC       FN1=FC2(k)            !!YV
       PN=PN*3.14/180.
       write(3,9)ih,ik,il,NS,NE,FO1(k),FN1(k),PN1(k)*3.14/180,PN,iflag  !!YV      
C      write(3,9)ih,ik,il,NS,NE,FO1(k),FN1(k),PN1(k)*3.14/180,iflag  !!YV 
CCC     write(*,9)ih,ik,il,NS,NE,FO1(k),FN1(k),FN,PN1(k)*3.14/180,PN    !!YV  
    9  format(3i4,2x,i5,2x,i4,2x,f8.2,2x,f8.2,2x,f8.2,2x,f8.2,i5) 
CC   32 continue 
   31 continue
C !!YV  33 continue
      PH_ERR=PH_ERR/INUM
