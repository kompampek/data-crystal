C     PROGRAM Q7 = normal free
      ...
      COMMON /MADD/LLX,LLL,LSX,LSL,LIM1,NOP,NC
      W = 1.
      NC(0)=-9
      mum=0
      mump=0
...
C     L-M, IS6
   ...
   11 continue
   12 continue
      nss = NINT(SSN/FLOAT(NTRIP))
      mss = NINT(SSM/FLOAT(NTRIP))
      write(*,*)ntrip,mum,nss,mss
      goto 25
    3 write(*,4)
...
C
      ...
      IX=NC(I)+NC(J)
      IF(IX.EQ.4) RETURN
      IF(IX.EQ.2) LLL=LLL+1
      IF(IX.GE.3.AND.IX.LE.5) LLX=LLX+1
      IF(IX.EQ.5) LSL=LSL+1
      IF(IX.GE.6) LSX=LSX+1
      RETURN
      END
