...
C     COMPARE 3 DIFFERENT Rmin TARGET FUNCTIONS, SAVE PHASE SETS
	  ...
	  integer, dimension(:), allocatable :: seed     !!YV
      real*8 x,y,z,rand
      dimension p(-mp:mp),tt(mt),an(mt),e(mp),TR(9,3),q(mp)
      dimension tnt(mt),r(4),true(-mp:mp)
      dimension cs(0:65536),sn(0:65536),xx(1500,3)
      COMMON /CYC/ ncyc,mcyc,nsets !!YV
...
C     READ SYM FILE
      ...
      do 1 i=1,mp
      read(1,*,end=29)ih(i),ik(i),il(i),ns,ne,a,b,c,true(i)
      nr(i)=13
      true(-i)=-true(i)
      do 22 j=2,NSYM
...
C     
       call random_seed(size=k)  !YV
       allocate(seed(1:k))        !!YV
      do 15 nsets=1,msets
...
C
      ...
      NEND = MATOM*(IRAN-1)/2
C      write(*,*)'NEND,IRAN=',NEND,IRAN  !!YV
      DO 28 I=1,NEND
   28 read(4,*)X
      DO 26 I=1,MATOM
...
C
      ...
      iseed=iy
c!YV      call srand(iseed)
c!YV      x=rand()                 !!YV
c!YV      x=rand()
c!YV     y=rand()
c!YV      z=rand()
        seed(:)=iy           !!YV
		call random_seed(put=seed)     !!YV
      	call random_number(harvest=rr)  !!YV
		x=rr                       !!YV
		call random_number(harvest=rr)  !!YV
		y=rr                      !!YV
		call random_number(harvest=rr)  !!YV
...
C
      AXE=90.
      mmcyc=0  !!YV
CC!YV      do 10 ncyc=1,mcyc
      do 10 ncyc=0,mcyc             !!!YV
      do 210 i=1,NREF
  210 q(i)=p(i)
      irest=mcyc-ncyc
      do 21 nit=0,iter
...
C     PARMETER SHIFT - INNER LOOP 16
   ...
   17 r(j)=0.
    ...
    9 if(NOP.eq.2.and.cost.gt.0.) r(j)=r(j)+1
   18 CONTINUE
      rmin=r(4)
      ind=0
...
C     CALCULATE Rmin, COSAV, TRIPLES Phase error
   20 continue
    ...
    3 Rold=Rold + a1*cost*cost
      ...
      wt=1.-cos(ar2)
      a3=a3+wt
      a1=a1+wt*acos(cos(ar1-ar2))
   35 a2=a2+wt*acos(cos(ar1+ar2))
      angle=a1
      angle=min(angle,a2)
      angle = 57.296*angle/a3
      MIST=1
      if(list.ne.0) mist=mod(ncyc,1)*LIST
CC     if(mist.EQ.0.) write(*,5)iran,ncyc,Rold,Cosav,angle,axe
...
C
      ...
      parameter (mp=9000)
      integer*4 jh(mp),jk(mp),jl(mp),IPR(9,3,3),nx(1500,16)
      integer*4 ny(1500,16),nz(1500,16)
      dimension e(mp),p(-mp:mp),TR(9,3)
      dimension cs(0:65536),sn(0:65536),xxx(1500,3)
C     integer*4 iseed          !!YV
C      integer time(8),seed(2)       !!YV
      integer :: values(1:8), k                      !!YV
      integer, dimension(:), allocatable :: seed1     !!YV
      COMMON /CYC/ ncyc,mcyc,nsets  !!YV
      COMMON /SIGNS/ cs,sn,xxx,NLIST
      COMMON /HKL/ e,p,tr,NREF,NTOTAL,jh,jk,jl,NSYM,ipr
C      m=1                !!YV
       cut=mcyc-3.          !!YV
CCC      call DATE_AND_TIME(values=time)   !!YV
       call date_and_time(values=values)   !!YV
       call random_seed(size=k)       !YV
       allocate(seed1(1:k))           !!YV
       seed1(:) = values(8)      !!YV

       iseed=nsets+ncyc          !!YV  
c       iseed=899*iseed          !!YV   
        iseed=19*iseed+697       !!YV
       iseed=iand(iseed,65535)  !!YV
cccc       seed1(:) = iseed            !!YV
c       call srand(iseed)     !!YV
c       seed1(:) =899*(ncyc+nsets)       !!YV
       call random_seed(put=seed1)  !!YV
c       do 8 i=1,NAT            !YV
c       do 9 j=1,NSYM           !YV 
c	   nx(i,j)=0                !YVc
c	   ny(i,j)=0                !YV
...
c    9 continue                 !YV
c    8 continue                 !YV 
C      seed(1)=time(4)*(360000*time(5)+6000*time(6)+100*time(7)+time(8))

cc         write(*,*)'seed====',seed1(9)     !!YV

       do 1 i=1,NAT
C      m=mod(i,2)*NLIST   !!YV
CCC     write(*,*)'mist, NLIST===',mist,NLIST !!YV!!!!!!
ccc       iseed=i+ncyc          !!YV  
C      iseed=899*iseed       !!YV
C      iseed=iand(iseed,65535)  !!YV
cc       call rand(iseed)     !!YV
       call random_number(harvest=rr)  !!YV
c       rr=rand()             !!YV
c	   rr=rand()             !!YV  
CCCCC       if(i.lt.20) write(*,*) 'i,rr==',i,rr   !!YV
       if(ncyc.eq.0) go to 55   !!YV  
       if(rr.gt.0.8.and.ncyc.le.cut) go to 1 !!YV
55      xx=xxx(i,1)
      yy=xxx(i,2)
      zz=xxx(i,3)
      do 2 j=1,NSYM
    ...
    1 continue
      ...
      do 3 k=1,NSYM
      do 4 j=1,NAT
      narg = ih*nx(j,k) + ik*ny(j,k) + il*nz(j,k)
      narg=iand(narg,65535)
      aa = aa + cs(narg)
    4 bb = bb + sn(narg)
    3 continue
...
C     Zero P(jk,iz) array before calculating next section on x
   ...
   10 r(iy,iz,ixx)=fx(iy)
    7 continue
      ...
      call cpu_time(pk0)
      if(ix.ge.1) call pkpick(ix-1,mid,npeaks,rlast,frac)
      call cpu_time(pk1)
      pks=pks+pk1-pk0
      mid=ixx
    1 continue
...
C     
    ...
    1 M2=M2/2
      ...
      END
C
      SUBROUTINE PKPICK(ixm1,MID,npeaks,rlast,frac)
      parameter (NPAR=256)
...
C     perform Rollet's ellipsoidal fit procedure
      ...
      if(ax(1).lt.-0.0001.or.ax(1).gt.frac) goto 6
      N=N+1
