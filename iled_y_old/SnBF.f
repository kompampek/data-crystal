      PROGRAM SnB3
C     COMPARE SnB and SnB2, usefree or P10 wt'd tau values
      parameter (mp=9000,mt=200000,mp2=18001,mt3=3*mt)
      character fil*6
      BYTE nt(mt),np(-mp:mp),true(-mp:mp),narg
      BYTE IPR(9,3,3),ih(mp),ik(mp),il(mp),nr(mp)
      integer*4 iseed,MM(0:mp),n0,nm(mt3),M(mp),NN(3,mt)
      real*8 x,y,z,rand
      dimension tt(mt),an(mt),e(mp),TR(9,3),r(4)
      dimension cs(-128:128),sn(-128:128),xx(1500,3)
      COMMON /SIGNS/ cs,sn,xx,NLIST
      COMMON /HKL/ e,tr,NREF,NTOTAL,ih,ik,il,NSYM,np,ipr,nr
...
C     READ SYM FILE
      ...
      do 1 i=1,mp
      read(1,*,end=29)ih(i),ik(i),il(i),ns,ne,a,b,c,npp
      nr(i)=13
      t=0.128*npp/PI
      true(i)=NINT(t)
      true(-i)=-true(i)
...
C     READ TRIPLES FILE
      ...
      ntrip=0
      do 33 i=1,mt
      read(1,*,end=2)a1,m1,m2,n1,n2,n3,a2,T
      if(junk.lt.0) T=a2
      if(m1.eq.7) goto 33
      if(junk.ne.0) goto 333
      t=0.
      a2=a1*a1
      a3=a1*a2
...
C
   ...
   31 ix=iran
    ...
    7 ix=iy
      MAD=NST
   30 call SFCLC(MAD,1)
c      write(79,779)(np(i),i=1,100)
c      write(80,779)(true(i),i=1,100)
c  779 format(20i4)    
      close(unit=1)
C
...
C
      LYNX=MCYC/10
      JANK=3.*LYNX
      if(JANK.LE.30) JANK=30
      if(JANK.gt.50) JANK=50
      AXE=90.
      do 10 ncyc=1,mcyc
      jinxz=mod(ncyc,LYNX)
      jinx=jinxz
      if(ncyc.lt.jank) jinx=0
      irest=mcyc-ncyc
      do 21 nit=0,iter
      if(nit.eq.0) goto 20
C     PARMETER SHIFT - INNER LOOP 16
   ...
   17 r(j)=0.
      ...
      n2=nn(2,k)
      n3=nn(3,k)
      narg = np(nn(1,k))+np(n2)+np(n3)+nt(k)
      if(i.eq.-n2.or.i.eq.-n3) narg=-narg
      if(pip.eq.0.0.or.jinx.ne.1) goto 123
      cost=cs(narg)
      tip = (ttk+pip)/(pip+1)
      if(cost.ge.tip) goto 18
  123 CONTINUE      
      do 9 j=1,4
      narg=narg+64
      cost=cs(narg)
      ...
      if(NOP.ne.2) r(j)=r(j)+ank*(sint2 + (cost-ttk)**2 )
    9 continue
   18 CONTINUE
      rmin=r(4)
      ind=0
C     NOP=0 Rmin, NOP=1 A*((cos2-t)**2 +sin**2)     
      do 8 j=1,3
      if(irest.lt.5.and.nr(i).ne.13.and.j.ne.2) goto 8
      if(NOP.ne.2.and.r(j).gt.rmin) goto 8
      if(NOP.eq.2.and.r(j).lt.rmin) goto 8
      rmin=r(j)
      ind=j
    8 continue
      np(i)=np(i)+ind*64
...
C     CALCULATE Rmin, COSAV, TRIPLES Phase error
   20 continue
      ...
      do 3 i=1,ntrip
      a1=an(i)
      narg=np(nn(1,i))+np(nn(2,i))+np(nn(3,i))+nt(i)
      cost=cs(narg)
      Cosav=Cosav+a1*cost
      cost=tt(i)-cost
    3 Rold=Rold + a1*cost*cost
      ...
      a1=0.
      do 35 j=1,NREF
      it=ih(j)*i1 +ik(j)*i2 +il(j)*i3
      it=iand(it,1)
      narg=np(j)+i4*true(j)+it*128
      cost=cs(narg)
   35 a1=a1+acos(cost)
      a1=57.296*a1/nref
      if(a1.gt.a2) goto 335
      a2=a1
  335 CONTINUE  
      angle=a2
      if(angle.lt.axe) icycle=ncyc
      axe=min(axe,angle)
      MIST=1
      if(list.ne.0) mist=mod(ncyc,10)*LIST
      if(mist.EQ.0.) write(*,5)iran,ncyc,Rold,Cosav,angle,axe,icycle
    5 format(i7,i4,2f7.3,f6.1,f6.1,i5)
   ...
   21 CONTINUE 
      ...
      tmap=tmap+t2-t1
      ink=1
c      fomit=float(ncyc)/float(mcyc)
c      if(fomit.lt.0.25) ink=3
c      if(fomit.lt.0.75) ink=2
      if(jink.ne.0) ink=4-jinxz
      if(ncyc.le.JANK) ink=1
      if(JINK.eq.0.OR.JINXZ.ge.4) INK=1
      call SFCLC(NATOM,INK)
      call cpu_time(t3)
      tSF=tSF+t3-t2
   10 CONTINUE
  789 write(3,5)iran,kcyc,rold,cosav,angle,axe,icycle
c      write(13,5)iran,kcyc,rold,cosav,angle,axe
...
c   37 format(20f6.2)   
   ...
   15 continue
     ...
     *' FR =',f9.3,' PPK =',f9.3,' Minutes')
      ...
      END
C
      SUBROUTINE SFCLC(NAT,INK)
      parameter (mp=9000)
      byte jh(mp),jk(mp),jl(mp),IPR(9,3,3),np(-mp:mp),narg,nr(mp)
      integer*2 nx(1500,16),ny(1500,16),nz(1500,16),marg
      dimension e(mp),TR(9,3)
      dimension cs(-128:128),sn(-128:128),xxx(1500,3)
      COMMON /SIGNS/ cs,sn,xxx,NLIST
      COMMON /HKL/ e,tr,NREF,NTOTAL,jh,jk,jl,NSYM,np,ipr,nr
      PI=acos(-1.0)
      do 1 i=1,NAT
      xx=xxx(i,1)
      yy=xxx(i,2)
    ...
    1 continue
      ...
      do 3 k=1,NSYM
      do 4 j=1,NAT,INK
      marg = ih*nx(j,k) + ik*ny(j,k) + il*nz(j,k)
      narg=marg/256
      aa = aa + cs(narg)
    4 bb = bb + sn(narg)
    3 continue
...
C     RADIX-2 FFT  -  PLUS PEAK LISTING SUBROUTINES
      ...
      BYTE ih(mp),ik(mp),il(mp),IPR(9,3,3),ph(-mp:mp),nr(mp),narg
      integer*4 nmy(0:NPA),nmz(0:NPA)
      dimension r(-1:NPAR,-1:NPAR,3),t(4,4),tx(24,3),TR(9,3),tq(4,4)
      dimension e(mp),cs(-128:128),sn(-128:128),xx(1500,3)
      ...
      COMMON /HKL/ e,tr,NREF,NTOTAL,ih,ik,il,NSYM,ph,ipr,nr
      COMMON /SIGNS/ cs,sn,xx,NLIST
...
C     set up FFT arrays for bit reversal on first pass
   ...
   29 nh(jk,jl)=1
      ...
      mmk=min(jk,mmk)
      mml=min(jl,mml)
   28 f(jh,jk,jl) = e(i)*cmplx(cs(narg),sn(narg))
   26 continue
