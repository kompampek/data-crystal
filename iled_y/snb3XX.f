...
C     COMPARE 3 DIFFERENT Rmin TARGET FUNCTIONS, SAVE PHASE SETS
      ...
      dimension tnt(mt),r(4),rs(4),true(-mp:mp)
...
C     ix odd > 0 to scan random phase sets
      read(1,*)id,NAT,iter,mcyc,msets,list,MAT,NST,NOP,ESC,tq,tqq
      write(3,24)id,NAT,iter,mcyc,msets,list,MAT,NST,NOP,ESC,tq,tqq
   24 format(9i5,f5.1,2f7.3)
...
C     PARMETER SHIFT - INNER LOOP 16
   ...
   17 r(j)=0.
      ...
      cost=cs(narg)
      sint=sn(narg)
      if(nr(i).ne.13)sint=0.
      rs(j)=rs(j)+sank*sint
    9 r(j)=r(j)+ank*(cost-ttk)**2 
   18 CONTINUE
      do 579 j=1,4
  579 r(j)=r(j) +rs(j)**2
      rmin=r(4)
...
C     NOP=0 Rmin, NOP=1 A*((cos2-t)**2 +sin**2) , NOP=2 statistical    
    ...
    8 continue
      ...
      p(-i)=-p(i)
   16 continue
C     axe = average shift in p(i) from previous cycle   
      goto 21
C     CALCULATE Rmin, COSAV, TRIPLES Phase error
  ...
  211 axe=axe+acos(dp)*e(i)**2
    ...
    3 Rold=Rold + a1*cost*cost
      ...
      a2=0.
      do 35 i=1,NREF
      it=abs(ih(i)*i1+ik(i)*i2+il(i)*i3)
      it=iand(it,1)
      dp=cos(true(i)-i4*p(i)+it*PI)
   35 a2=a2+acos(dp)
      a2=57.296*a2/nref
      if(a2.gt.a1) goto 355
      a1=a2
      i5=i1
      i6=i2
      i7=i3
  ...
  355 CONTINUE 
      ...
      if(mist.EQ.0.)write(*,5)iran,ncyc,Rold,Cosav,angle,axe,i5,i6,i7,i8
    5 format(i7,i4,2f7.3,2f6.1,4i3)
  ...
  789 write(3,5)iran,kcyc,rold,cosav,angle,axe
   ...
   15 continue
     ...
     *' FR =',f9.3,' PPK =',f9.3,' Minutes')
      ...
      END
C
      SUBROUTINE SFCLC(NAT,ISEED,tqq)
      parameter (mp=9000)
