...
C     extensions E=.eee, T=.trp, A=.amb, Q=.nqt, P=.pks, X=.xyz     
      open(unit=2,file='fft1.dat',status='old',form='formatted')
      read(2,*)NSYM
      read(2,*)NSYM,VOL
      close(unit=2)
      pi = acos(-1.0)
...
c      if(r5.gt.4.) write(*,*)ncyc,r5
      ...
      if(mist.EQ.0.) write(*,5)iran,ncyc,Rold,Cosav,angle,r5
   ...
   21 CONTINUE 
      ...
      tSF=tSF+t3-t2
   10 CONTINUE
  789 write(3,5)iran,kcyc,rold,cosav,angle,r5
      write(13,5)iran,kcyc,rold,cosav,angle,r5
...
C
      ...
      do 1 i=1,NAT
      nx(i)=65536*amod(xxx(i,1),1.0)
      ny(i)=65536*amod(xxx(i,2),1.0)
      nz(i)=65536*amod(xxx(i,3),1.0)
    1 continue
      do 5 i=1,NREF
      ih=jh(i)
      ik=jk(i)
