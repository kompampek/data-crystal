      ...
      do 1 i=1,80
      ind(i)=0
      read(98,*,END=111)x(i),y(i),z(i)
    1 num=num+1
  111 open(1,file='fft0.dat')
      read(1,*)nx
      read(1,*)nsym
      do 2 i=1,4
    ...
    5 CONTINUE
      ...
      ang=57.296*acos(d)
      nall=nall+1
      if(ang.ge.95.0.and.ang.le.135.0) goto 112
      nbad=nbad+1
      ind(i)=ind(i)+1
      ind(ll(l))=ind(ll(l))+1
      ind(ll(m))=ind(ll(m))+1
  112 write(23,11)i,ll(l),ll(m),rr(l),rr(m),ang
   11 format(3i4,2f6.2,f6.1)   
    8 CONTINUE  
      nang=nang+1
    4 CONTINUE
      do 113 i=1,num
  113 write(24,11)i,ind(i)
      fig=float(nbad)/float(nall)
      write(*,*)num,fig
      rewind(98)
      do 114 i=1,num
      if(ind(i).gt.6) goto 114
