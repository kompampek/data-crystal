...
C     COMPARE 3 DIFFERENT Rmin TARGET FUNCTIONS, SAVE PHASE SETS
      ...
      dimension tnt(mt),r(4),true(mp),RHO(1500)
...
C
      ...
      irest=mcyc-ncyc
      lop = mod(ncyc,10)
      if(lop.gt.0.and.lop.le.2) lop=50
      do 21 nit=0,iter
      if(nit.eq.0) goto 20
C     PARMETER SHIFT - INNER LOOP 16
   ...
   17 r(j)=0.
      ...
      sint=sn(narg)**2
      if(lop.eq.50) sint=0.
    9 r(j)=r(j)+ank*(sint + (cs(narg)-ttk)**2 )
   18 CONTINUE
      rmin=r(4)
      ind=0
...
C     CALCULATE Rmin, COSAV, TRIPLES Phase error
  ...
  355 a2=a2+acos(dp)
   ...
   35 CONTINUE   
      angle = a1
      if(ncyc.gt.5) axe=min(axe,angle)
      MIST=1
...
C
      ...
      dimension e(mp),p(-mp:mp),TR(9,3),RHO(1500)
...
C
      ...
      dimension cs(-0:65536),sn(0:65536),xxx(1500,3),RHO(1500)
...
C     perform Rollet's ellipsoidal fit procedure
      ...
      if(ax(1).lt.-0.0001.or.ax(1).gt.Xlim) goto 6
      N=N+1
